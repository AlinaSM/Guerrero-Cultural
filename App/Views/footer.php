<footer class="container-fluid  footer-bg-color">
    <div class="row ">
        <div class="col-xl-6  col-lg-6  col-md-6  col-sm-12  col-12  p-xl-5  p-lg-4  p-md-4  p-sm-3 p-3">
            Desarrollado por: Alina Salinas Mendoza. <br>
            Estudiante de Ingenieria en Sistemas Computacionales. <br>
            Instituto Tecnologico de Acapulco. <br>   
        </div>
        <div class="col-xl-6  col-lg-6  col-md-6  col-sm-12  col-12  p-xl-5  p-lg-4  p-md-4  p-sm-3 p-3">
            Sistema de Información Cultural del Estado de Guerrero.
        </div>
        
    </div>

</footer>

<!-- Cargando los archivos Js para Bootstrap-->
<script src='../Resources/js/jquery.slim.min.js'></script>
<script src='../Resources/js/popper.min.js'></script>
<script src='../Resources/js/bootstrap.min.js'></script>
</body>
</html>